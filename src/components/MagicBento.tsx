...
const cardData: BentoCardProps[] = [
  { color: '#060010', title: 'Mission Sim Lab', description: 'Run NASA-style scenarios and flight rules.', label: 'Simulations' },
  { color: '#060010', title: 'Model Rockets', description: 'Design, build, and launch test vehicles.', label: 'Launch' },
  { color: '#060010', title: 'Robotics & Rovers', description: 'Autonomous nav, sensors, and controls.', label: 'Robotics' },
  { color: '#060010', title: 'Orbital Mechanics', description: 'Transfer windows, Δv budgeting, trajectories.', label: 'Astro' },
  { color: '#060010', title: 'Data & Coding', description: 'Python notebooks, telemetry parsing, viz.', label: 'Software' },
  { color: '#060010', title: 'Team Ops', description: 'Roles, comms, and go/no-go readiness.', label: 'Crew' },
];
...
const BentoCardGrid: React.FC<{ children: React.ReactNode; gridRef?: React.RefObject<HTMLDivElement> }> = ({ children, gridRef }) => (
  <div className="bento-section" style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }} ref={gridRef}>
    {children}
  </div>
);
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  const effectiveParticleCount = shouldDisableAnimations ? Math.min(6, particleCount) : particleCount;
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive">
          {cardData.map((card, index) => {
            const baseClassName = `card card--interactive ${enableBorderGlow ? 'card--border-glow' : ''}`;
